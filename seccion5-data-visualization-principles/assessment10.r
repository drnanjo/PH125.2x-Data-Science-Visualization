...
population by state. We are going to again modify this plot to look at smallpox cases 
instead.
...
For this exercise, we have provided less sample code - you can take a look at the previous 
...
filter(!is.na(population)) %>%    group_by(year, disease) %>%
summarize(rate = sum(count)/sum(population)*10000) %>%
  ggplot(aes(year, rate, color = disease)) + 
  geom_line()
